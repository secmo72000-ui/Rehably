...
import type { ModalityItem, LibraryListResponse } from '@/domains/library/library.types';
...
export default function ModalitiesPage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const result: LibraryListResponse<ModalityItem> = await clinicLibraryService.getModalities({
        page,
        pageSize: perPage,
              ...
              {loading ? (
                ...
                items.map(item => (
                  <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-600 whitespace-nowrap text-xs">{item.therapeuticCategory}</td>
                    <td className="px-4 py-3 text-gray-600 text-xs max-w-[200px] truncate">{item.mainGoal}</td>
                    <td className="px-4 py-3 text-gray-600 text-xs text-center">{item.minSessions ?? '—'}</td>
                    <td className="px-4 py-3 text-gray-600 text-xs text-center">{item.maxSessions ?? '—'}</td>
