...
import type { AssessmentDto, LibraryListResponse } from '@/domains/library/library.types';

const timePointMap: Record<number, { label: string; cls: string }> = {
  0: { label: 'أولي',    cls: 'bg-purple-50 text-purple-600' },
  1: { label: 'منتصف',  cls: 'bg-yellow-50 text-yellow-600' },
  2: { label: 'نهائي',  cls: 'bg-green-50 text-green-600' },
};
...
export default function AssessmentsPage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const result: LibraryListResponse<AssessmentDto> = await clinicLibraryService.getAssessments({
        page,
        pageSize: perPage,
  ...
  const columns = ['الكود', 'الاسم', 'وقت التطبيق', 'الوصف', 'النوع'];
          ...
          <table className="w-full text-sm" dir="rtl">
            ...
            <tbody>
              {loading ? (
                ...
                items.map(item => {
                  const tp = timePointMap[item.timePoint] ?? { label: String(item.timePoint), cls: 'bg-gray-100 text-gray-500' };
                  return (
                    <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold ${tp.cls}`}>
                          {tp.label}
                        </span>
                      </td>
